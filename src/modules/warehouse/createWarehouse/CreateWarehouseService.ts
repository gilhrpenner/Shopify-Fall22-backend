...
import { createWarehouseValidation } from '@shared/validations/warehouse.validations';
...
export class CreateWarehouseService {
    ...
    async execute(payload: ICreateWarehouseRequestDTO): Promise<Warehouse> {
        const { error: invalidInput } = createWarehouseValidation(payload);
        if (invalidInput) {
